...
class ThemeMode {
  ...
  private getParamName = (postfix: string): string => {
    const ktName = document.body.hasAttribute("data-mz-name");
  ...
  public setMode = (mode: Mode, menuMode: Mode | ""): void => {
    ...
    const activeMenuItem: HTMLElement | null =
      this.menu?.querySelector(
        '[data-mz-element="mode"][data-mz-value="' + menuMode + '"]'
      ) || null;

    // Enable switching state
    this.element?.setAttribute("data-mz-theme-mode-switching", "true");
    ...
    const self = this;
    setTimeout(function () {
      self.element?.removeAttribute("data-mz-theme-mode-switching");
    }, 300);
  ...
  public getMenuMode = (): Mode | "" => {
    ...
    const menuItem = this.menu?.querySelector(
      '.active[data-mz-element="mode"]'
    );
    const dataKTValue = menuItem?.getAttribute("data-mz-value");
    if (dataKTValue) {
  ...
  private getActiveMenuItem = (): HTMLElement | null => {
    return (
      this.menu?.querySelector(
        '[data-mz-element="mode"][data-mz-value="' + this.getMenuMode() + '"]'
      ) || null
    );
  };
  ...
  private setActiveMenuItem = (item: HTMLElement): void => {
    const menuModeParam = this.getParamName("menu");
    const menuMode = item.getAttribute("data-mz-value");
  ...
  private handleMenu = (): void => {
    this.menu
      ?.querySelectorAll<HTMLElement>('[data-mz-element="mode"]')
      ?.forEach((item: HTMLElement) => {
        item.addEventListener("click", (e) => {
          e.preventDefault();

          const menuMode: string | null = item.getAttribute("data-mz-value");
  ...
  public flipImages = () => {
    document
      .querySelectorAll<HTMLElement>("[data-mz-img-dark]")
      ?.forEach((item: HTMLElement) => {
        if (item.tagName === "IMG") {
          if (
            this.getMode() === "dark" &&
            item.hasAttribute("data-mz-img-dark")
          ) {
            item.setAttribute(
              "data-mz-img-light",
            ...
            item.setAttribute(
              "src",
              item.getAttribute("data-mz-img-dark") || ""
            );
          } else if (
            this.getMode() === "light" &&
            item.hasAttribute("data-mz-img-light")
          ) {
            item.setAttribute(
              "data-mz-img-dark",
              item.getAttribute("src") || ""
            ...
            item.setAttribute(
              "src",
              item.getAttribute("data-mz-img-light") || ""
            );
          }
        } else {
          if (
            ...
            item.style.backgroundImage =
              "url('" + item.getAttribute("data-mz-img-dark") + "')";
          } else if (
            this.getMode() === "light" &&
            item.hasAttribute("data-mz-img-light")
          ) {
            ...
            item.style.backgroundImage =
              "url('" + item.getAttribute("data-mz-img-light") + "')";
          }
        }
      });
  };
  ...
  public init = () => {
    this.menu = document.querySelector<HTMLElement>(
      '[data-mz-element="theme-mode-menu"]'
    );

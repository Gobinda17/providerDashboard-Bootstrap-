...
class StepperComponent {
  ...
  private getStepContent = (index: number) => {
    const content = this.element.querySelectorAll(
      '[data-mz-stepper-element="content"]'
    );
    if (!content) {
      return false;
    }
  ...
  private refreshUI = () => {
    ...
    const elements = this.element.querySelectorAll(
      '[data-mz-stepper-element="nav"], [data-mz-stepper-element="content"], [data-mz-stepper-element="info"]'
    );
    ...
    for (let i = 0, len = elements.length; i < len; i++) {
      ...
      if (index === this.currentStepIndex) {
        ...
        if (
          ...
          element.getAttribute("data-mz-stepper-element") === "content"
        ) {
  ...
  public static bootstrap(attr: string = "[data-mz-stepper]") {

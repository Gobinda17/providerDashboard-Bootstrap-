...
class ScrollTopComponent {
  ...
  private _scroll = () => {
    ...
    if (pos > offset) {
      if (!document.body.hasAttribute("data-mz-scrolltop")) {
        document.body.setAttribute("data-mz-scrolltop", "on");
      }
    } else {
      if (document.body.hasAttribute("data-mz-scrolltop")) {
        document.body.removeAttribute("data-mz-scrolltop");
      }
    }
  };
  ...
  private _getOption = (name: string) => {
    const attr = this.element.getAttribute(`data-mz-scrolltop-${name}`);
    if (attr) {
  ...
  public static bootstrap = () => {
    ScrollTopComponent.createInstances('[data-mz-scrolltop="true"]');
  };

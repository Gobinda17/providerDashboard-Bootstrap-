...
class DrawerComponent {
  ...
  constructor(_element: HTMLElement, options: DrawerOptions) {
    ...
    this.name = this.element.getAttribute("data-mz-drawer-name") || "";
    this.shown = false;
  ...
  private _update = () => {
    ...
    const bodyCanvasAttr = String(
      document.body.getAttribute(`data-mz-drawer-${this.name}-`)
    );
  ...
  private _getOption = (name: string) => {
    const attr = this.element.getAttribute(`data-mz-drawer-${name}`);
    if (attr) {
  ...
  private _toggle = () => {
    if (EventHandlerUtil.trigger(this.element, "mz.drawer.toggle") === false) {
      return;
    }
    ...
    EventHandlerUtil.trigger(this.element, "mz.drawer.toggled");
  };

  private _hide = () => {
    if (EventHandlerUtil.trigger(this.element, "mz.drawer.hide") === false) {
      return;
    }

    this.shown = false;
    this._deleteOverlay();
    document.body.removeAttribute(`data-mz-drawer-${this.name}`);
    document.body.removeAttribute(`data-mz-drawer`);
    this.element.classList.remove(`${this.options.baseClass}-on`);
    if (this.toggleElement != null) {
      this.toggleElement.classList.remove("active");
    }

    EventHandlerUtil.trigger(this.element, "mz.drawer.after.hidden");
  };

  private _show = () => {
    if (EventHandlerUtil.trigger(this.element, "mz.drawer.show") === false) {
      return;
    }

    this.shown = true;
    this._createOverlay();
    document.body.setAttribute(`data-mz-drawer-${this.name}`, "on");
    document.body.setAttribute("data-mz-drawer", "on");
    this.element.classList.add(`${this.options.baseClass}-on`);
    if (this.toggleElement !== null) {
      this.toggleElement.classList.add("active");
    }

    EventHandlerUtil.trigger(this.element, "mz.drawer.shown");
  };
  ...
  public static handleDismiss = () => {
    ...
    DOMEventHandlerUtil.on(
      document.body,
      '[data-mz-drawer-dismiss="true"]',
      "click",
      () => {
        /* @ts-ignore */
        const element = this.closest('[data-mz-drawer="true"]');
        if (element) {
  ...
  public static bootstrap = () => {
    DrawerComponent.createInstances('[data-mz-drawer="true"]');
